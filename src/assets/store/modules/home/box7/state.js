export default {
    home_page_box_7: {
        image: require('@/assets/images/form_box7.png'),
        title: "Давайте обсудим<br>" +
                 " ваш проект и<br>" +
                 " подготовим его<br> расчет",
        // text1: "Мы рассчитаем точную стоимость<br>" +
        //         " в течение 24 часов, подскажем,<br>" +
        //         " на чем вы можете сэкономить<br>" +
        //         " и расскажем все о наших возможностях.",
        // text2: "Если у вас уже есть предложение от другой компании,<br>" +
        //          " прикрепите его к форме. Мы дадим свое предложение<br>" +
        //          " по наличию, цене и срокам поставки.<br>" +
        //          "<b style='color:#19539F'>Просто попробуйте, уверены, мы вас приятно удивим.</b>",
        text1: "Мы рассчитаем точную стоимость" +
            " в течение 24 часов, подскажем," +
            " на чем вы можете сэкономить" +
            " и расскажем все о наших возможностях.",
        text2: "Если у вас уже есть предложение от другой компании," +
            " прикрепите его к форме. Мы дадим свое предложение" +
            " по наличию, цене и срокам поставки. " +
            "<b style='color:#19539F'>Просто попробуйте, уверены, мы вас приятно удивим.",

        form_title: "Получите предложение<br>" +
                    "на поставку металлопроката",
        form_labels: {
            "name": "Как к вам обращаться?",
            "telefon": "Введите телефон",
            "email": "Куда отправить прайс и каталог?"
            },
        form_placeholders: {
            "name": "Ваше имя",
            "telefon": "+ 7 ( _ _ _ ) - _ _ _ - _ _ - _ _",
            "email": "E-mail"
        },

        form_subtitle1: "Прикрепите файлы<br>" +
                        " (заказ,спецификацию, смету, если есть)",
        form_subtitle2: "Или напишите нам на почту <a href='mailto:info@tpk-gefest.ru' style='color:#213252;text-decoration:none'><b>info@tpk-gefest.ru</b></a>",
        form_image: require('@/assets/images/icon-skrepka.png'),
        text_btn: "Узнать стоимость",
        text_policy: "Нажимая на кнопку, вы даете согласие на<br>" +
                     " обработку своих персональных данных<br>" +
                     " и соглашаетесь с <a href='/policy' style='color:#2D72CD; text-decoration: none'> Политикой конфиденциальности</a>"


    }
}